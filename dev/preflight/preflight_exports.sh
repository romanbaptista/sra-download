...
echo
echo "Generating export snapshot: SBATCH_EXPORTS"
EXPORT_PAIRS=()

for entry in "${EXPORT_ARRAY[@]}"; do
    variable_check_nonempty "${entry}" || error_exit "Variable is empty or not set: ${entry}"
    EXPORT_PAIRS+=("${entry}=${!entry}")
done

(IFS=,; SBATCH_EXPORTS="${EXPORT_PAIRS[*]}")
export SBATCH_EXPORTS
echo "Snapshot complete"
echo "SBATCH_EXPORTS: ${SBATCH_EXPORTS}"
